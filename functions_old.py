...
#------------------------------------ INITIAL VARIABLES -----------------------------------------
viewdistance = 10        # camera viewpoint distance from origin (in m). Choose 10 for good view
...
def rotation_matrix(angles, drone=False):
    ...
    sigma = angles[0]
    theta = angles[1]
    return np.array([[cos(si)*cos(theta)-sin(sigma)*sin(si)*sin(theta),   -cos(sigma)*sin(si),    cos(si)*sin(theta)+cos(theta)*sin(sigma)*sin(si)],
                     [cos(theta)*sin(si)+cos(si)*sin(sigma)*sin(theta),    cos(sigma)*cos(si),    sin(si)*sin(theta)-cos(si)*cos(theta)*sin(sigma)],
                     [-cos(sigma)*sin(theta),                              sin(sigma),            cos(sigma)*cos(theta)]])
