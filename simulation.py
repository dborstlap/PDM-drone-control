########### This file can simulate and visualize the drone in pygame ###############################
...
class drone:
    ...
    def display(self):
        ...
        pg.draw.line(scr, color.cyan, projection(self.pos,view_angles,origin,scale), 
                                      projection(self.pos+np.dot(self.drone_rotation_matrix,[0.1,0,0]),view_angles,origin,scale), 5)
  
        # draw drone diagonals
        pg.draw.line(scr, color.red, projection(rotor_pos[0],view_angles,origin,scale), projection(rotor_pos[1],view_angles,origin,scale), 5)
        pg.draw.line(scr, color.red, projection(rotor_pos[2],view_angles,origin,scale), projection(rotor_pos[3],view_angles,origin,scale), 5)
        ...
        for pos in rotor_pos:
            for theta in range(40):
                ...
                pg.draw.circle(scr, color.white, projected_circle_pos, 1)
...
class meteorite:
    ...
    def display(self):
        ...
        pg.draw.circle(scr, color.grey, screen_pos, self.size*scale*depth_scale(self.pos,view_angles, scale))
...
while running:
    ...
    for i,m in enumerate(meteorites):
        if np.linalg.norm(drone1.pos-m.pos) < 1:
            meteorites.pop(i)
            colission = True
        localtime = display_explosion(scale, colission, drone1.pos, dt, localtime)
        colission = False



    # meteorites
    [m.display() for m in meteorites]
    ...
    pg.draw.line(scr, color.lime, projection([0,0,0],view_angles,origin,scale), projection([1,0,0],view_angles,origin,scale), 4)
    pg.draw.line(scr, color.lime, projection([0,0,0],view_angles,origin,scale), projection([0,1,0],view_angles,origin,scale), 4)
    pg.draw.line(scr, color.lime, projection([0,0,0],view_angles,origin,scale), projection([0,0,1],view_angles,origin,scale), 4)

    text_font = text_fonts[20]
    x_axis_text, y_axis_text, z_axis_text = text_font.render('X', True, color.lightblue), text_font.render('Y', True, color.lightblue), text_font.render('Z', True, color.lightblue)
