...
draw_points = True                                      # draws waypoints, only for manual_control = False
draw_path = True                                        # draws the path the drone flies                 
draw_gates = True                                       # draws a gate at the origin. Has no function as of now.      
...
while running:
    ...
    if draw_points == True and manual_control == False:
    ...
    for pos in rotor_pos:
        for theta in range(40):
            ...
            pg.draw.circle(scr, drone_rotor_color, projected_circle_pos, 1)
